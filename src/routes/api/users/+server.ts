...
export async function POST ({ request }: RequestEvent) {
      ...
      if (!username || !password || !name || !role || typeof username != "string" || typeof password != "string" || typeof name != "string" || typeof role != "string")
      ...
      try {
            ...
            return new Response(JSON.stringify({message: `User ${user.username} with role ${user.role} created!`}), { status: 200 });
      } catch (error) {
...
export async function DELETE ({request} : RequestEvent) {
      ...
      try {
            ...
            if (!userQuery) return new Response(JSON.stringify({message: "User not found"}), { status: 400 });

            const user = await auth.deleteUser(userQuery.id)
            return new Response(JSON.stringify({message: `User ${userQuery.username} deleted!`}), { status: 200 });
      } catch (error) {
            if (error instanceof LuciaError && error.message === "AUTH_DUPLICATE_KEY_ID") {
